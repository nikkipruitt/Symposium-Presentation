...
to make-tiger [vision-mean vision-stdev]
  ...
  ifelse vision-stdev = 0 [
    ...
    set vision-distance random-normal vision-mean vision-stdev
    if vision-distance < 1 [
      set vision-distance 1
    ]
  ]
  set vision-angle calc-vision-angle
...
to-report calc-vision-angle
  set vision-arclength 2 * vision-area / vision-distance
  report 360 * vision-area / (pi * vision-distance * vision-distance)
end
...
to draw-vision-cone
  hatch 1 [
   ...
   lt 90
   let degs 0
   while [degs < vision-angle] [
     fd vision-arclength / vision-angle
     lt 1
     set degs degs + 1
   ]
   lt 90
...
to show-vision-cone
  hatch 1 [
   let theta vision-angle / 2
   let c vision-distance / cos(theta)
   let b sqrt(c * c - vision-distance * vision-distance)
   set color white
   rt theta
   pd
   fd c
...
to record
  movie-start "tigers-hunting2.mov"
  set recording true
end
